...
with st.form("loan_form"):
    applicant_income = st.number_input(
        "Monthly Income", min_value=0.0, value=8000.0, step=500.0
    )
    ...
    loan_amount = st.number_input(
        "Loan Amount Requested", min_value=0.0, value=10000.0, step=500.0
    )
...
# The model was trained on a dataset with a limited numeric range.
# Values outside this range are clipped before scoring so the model
# never sees wildly out-of-distribution numbers (which caused broken
# predictions). The user can still type any real-world number above.
TRAIN_RANGES = {
    "Applicant_Income": (2009.0, 19988.0),
    "Credit_Score": (550, 799),
    "DTI_Ratio": (0.10, 0.60),
    "Loan_Amount": (1015.0, 39995.0),
}

if submitted:
    # Clip to the range the model was actually trained on, so extreme
    # values don't produce nonsensical predictions. We still show the
    # user's real number in a note if we had to clip it.
    clipped_income = min(max(applicant_income, TRAIN_RANGES["Applicant_Income"][0]), TRAIN_RANGES["Applicant_Income"][1])
    clipped_credit_score = min(max(credit_score, TRAIN_RANGES["Credit_Score"][0]), TRAIN_RANGES["Credit_Score"][1])
    clipped_dti = min(max(dti_ratio, TRAIN_RANGES["DTI_Ratio"][0]), TRAIN_RANGES["DTI_Ratio"][1])
    clipped_loan_amount = min(max(loan_amount, TRAIN_RANGES["Loan_Amount"][0]), TRAIN_RANGES["Loan_Amount"][1])

    was_clipped = (
        clipped_income != applicant_income or
        clipped_credit_score != credit_score or
        clipped_dti != dti_ratio or
        clipped_loan_amount != loan_amount
    )

    raw = pd.DataFrame([{
        "Applicant_Income": clipped_income,
    ...
    if was_clipped:
        st.caption(
            "Note: this model was trained on a limited dataset "
            "(income up to ₹20,000, credit score up to 799, loan amount up to ₹40,000). "
            "Your entered value was outside that range, so the prediction is based on the "
            "closest value the model has learned from."
        )
