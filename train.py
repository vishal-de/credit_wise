...
# ---------- 8. Compute defaults for fields we hide from the simplified UI ----------
raw_df = pd.read_csv("loan_approval_data.csv")
defaults = {
    "Coapplicant_Income": float(raw_df["Coapplicant_Income"].mean()),
    "Age": int(raw_df["Age"].mean()),
    "Dependents": int(raw_df["Dependents"].mode()[0]),
    "Existing_Loans": int(raw_df["Existing_Loans"].mode()[0]),
    "Savings": float(raw_df["Savings"].mean()),
    "Collateral_Value": float(raw_df["Collateral_Value"].mean()),
    "Marital_Status": raw_df["Marital_Status"].mode()[0],
    "Property_Area": raw_df["Property_Area"].mode()[0],
    "Gender": raw_df["Gender"].mode()[0],
    "Employer_Category": raw_df["Employer_Category"].mode()[0],
    "Education_Level": raw_df["Education_Level"].mode()[0],
}

# ---------- 9. Save everything the app needs ----------
joblib.dump({
    "model": model,
    "scaler": scaler,
    "ohe": ohe,
